...
public class ChartPlotterRouteValidationTest {
	...
	public void freeDepartureSearchMatchesIndependentDijkstra() {
		...
		for (int trial = 0; trial < 30; trial++) {
			...
			for (int weight : new int[]{100, 110, 140}) {
				String context = "trial=" + trial + " speed=" + speed + " weight=" + weight;
				...
				assertNotNull(context, search.terrain);
				if (expected == Integer.MAX_VALUE) assertEquals(context, ChartPlotterRoute.NO_ROUTE, result.status);
				else {
					assertEquals(context, ChartPlotterRoute.OK, result.status);
					assertTrue(context + " cost=" + search.routeCost + " optimum=" + expected, search.routeCost >= expected && search.routeCost <= (long) expected * weight / 100);
					assertEquals(context, search.routeCost, cost(result, search.motion, bias));
				}
			}
		}
	}
	@Test
	public void openWaterRoutesReachTheExactWaypointAtDifferentSpeeds() {
		...
		for (double speed : new double[]{0.5, 1, 2, 3, 5}) for (int[] target : new int[][]{{40, 5}, {100, 1}}) {
			...
			assertEquals(target[0], route.x[route.n - 1]);
			assertEquals(target[1], route.y[route.n - 1]);
	...
	public void terrainLowerBoundIsConsistentForAcceptedHullMoves() {
		...
		ChartPlotterRouteDistances distance = new ChartPlotterRouteDistances(terrain, hull, 0, 16, 0, 0, -12, 0, ChartPlotterRouteTarget.create(data, 0, 16, () -> false), () -> false);
