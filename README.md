# Chart Plotter
Collision-aware sailing navigation helper for viewing your current and projected courses and planning a route to a destination.

## Features
- Draw current and projected sailing courses in the world view, minimap, and/or world map.
- Chart a course to a destination from the world map.
...
- `chart`: Data containing Java `Integer` values `x` and `y` from `WorldPoint.getX()` and `WorldPoint.getY()`, each in `0..16383`. These are standard RuneScape tiles, not Sailing subtiles. Accepted only while aboard a boat and at least one charted-line overlay is enabled.
- `clear`: No data fields; clears the current destination.
